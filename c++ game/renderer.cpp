...
static void draw_rect(float x, float y, float half_x, float half_y, u32 color) {
...
static void draw_text(const char* text, float x, float y, float size, u32 color) {
	...
	float original_y = y;
	while (*text) {
		if (*text != 32) { //for spaces in string
			float original_x = x;
...
static void draw_score(int score, float x, float y, float size, u32 color) {
	float half_size = size * .5f;

	bool drew_score = false;
	while (score || !drew_score) {
		drew_score = true;
